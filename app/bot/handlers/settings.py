...
async def process_save_click(
    ...
    user_lang = data.get("user_lang")
    if not user_lang:
        await callback.answer(text=i18n.get("no_lang_selected", "Выберите язык"), show_alert=True)
        return
    
    try:
        await update_user_lang(
            conn, language=user_lang, user_id=callback.from_user.id
        )
        await callback.message.edit_text(text=i18n.get("lang_saved"))
    except Exception as e:
        logger.error(f"Failed to update user language: {e}")
        await callback.answer(text=i18n.get("error_occurred", "Произошла ошибка"), show_alert=True)
        return
